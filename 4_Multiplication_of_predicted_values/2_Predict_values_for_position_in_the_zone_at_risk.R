...
for (i in 1:length(Liste_Sp)){
  ...
  BestModel <- readRDS(paste("./bat-road-collision-risks/SAVED MODELS/", Sp, "_Risk_BestModel.rds", sep=""))
  ...
  if(grepl("D_houppier", formula(BestModel)[3])){
    ...
    pr1=data.frame("D_houppier_scaled"=rep(newdata$D_houppier,20000),predictedRaw)
    ...
    write.table(as.data.frame(PredictTable),paste("./bat-road-collision-risks/SIM_PREDICT/", Sp,"Risk_D_houppier_predict_sim_table",".csv",sep=""),sep=";",row.names=F)
  ...
  if(grepl("N_vehi_nuit_1200", formula(BestModel)[3])){
    ...
    colnames(PredictTable)[1]="Species"
    write.table(as.data.frame(PredictTable),paste("./bat-road-collision-risks/SIM_PREDICT/", Sp,"Risk_N_vehi_nuit_1200_predict_sim_table",".csv",sep=""),sep=";",row.names=F)
  ...
  if(grepl("H_veg", formula(BestModel)[3])){
    ...
    colnames(PredictTable)[1]="Species"
    write.table(as.data.frame(PredictTable),paste("./bat-road-collision-risks/SIM_PREDICT/", Sp,"Risk_H_veg_predict_sim_table",".csv",sep=""),sep=";",row.names=F)
  ...
  if(grepl("Paysage", formula(BestModel)[3])){
    ...
    colnames(PredictTable)[1]="Species"
    write.table(as.data.frame(PredictTable),paste("./bat-road-collision-risks/SIM_PREDICT/", Sp,"Risk_Paysage_predict_sim_table",".csv",sep=""),sep=";",row.names=F)
  ...
  if(grepl("JJulien", formula(BestModel)[3])){
    ...
    colnames(PredictTable)[1]="Species"
    write.table(as.data.frame(PredictTable),paste("./bat-road-collision-risks/SIM_PREDICT/", Sp,"Risk_JJulien_predict_sim_table",".csv",sep=""),sep=";",row.names=F)
    
  }
  
  ############################################################# N_vehi_nuit_1200:Paysage ####
  
  if(grepl("N_vehi_nuit_1200:Paysage", formula(BestModel)[3])){
    print("N_vehi_nuit_1200:Paysage")
    # Create simulated table
    newdata = data.frame(JJulien_scaled=0,
                         Paysage=c("NV","F", "FE", "DPT","SPT","PT"),
                         D_houppier_scaled= 0,
                         N_vehi_nuit_1200_scaled= rep((c(-30:30)/10), 6),
                         H_veg_scaled=0,
                         Site="bla",
                         SP="bli")
    ...
    pr6=data.frame("N_vehi_nuit_1200_scaled"=rep(newdata$N_vehi_nuit_1200_scaled,20000),"Paysage"=rep(newdata$Paysage,20000),
                   predictedRaw)
    
    # Backtransformation of response
    pr6$predicted=1/(1+exp(-pr6$predictedRaw))
    
    # Backtransform before scaling
    pr6$N_vehi_nuit_1200=((pr6$N_vehi_nuit_1200_scaled*sd(sqrt(Variables2$N_vehi_nuit_1200)))+mean(sqrt(Variables2$N_vehi_nuit_1200)))^2
    
    # Table
    PredictTable=cbind(rep(Sp,nrow(pr6)),as.data.frame(pr6))
    colnames(PredictTable)[1]="Species"
    write.table(as.data.frame(PredictTable),paste("./bat-road-collision-risks/SIM_PREDICT/",  Sp,"Risk_INTER_N_vehi_nuit_1200_Paysage_predict_sim_table",".csv",sep=""),sep=";",row.names=F)
