...
for (i in 1:length(Liste_Sp)){
  ...
  BestModel <- readRDS(paste("./bat-road-collision-risks/SAVED MODELS/", Sp, "_Acti_BestModel.rds", sep=""))
  ...
  if(grepl("D_houppier", formula(BestModel)[3])){
    ...
    pr1$predicted=exp(pr1$predictedRaw)
    ...
    write.table(as.data.frame(PredictTable),paste("./bat-road-collision-risks/SIM_PREDICT/", Sp,"Acti_D_houppier_predict_sim_table",".csv",sep=""),sep=";",row.names=F)
  ...
  if(grepl("N_vehi_nuit_1200", formula(BestModel)[3])){
    ...
    # Backtransformation of response
    pr2$predicted=exp(pr2$predictedRaw)
    ...
    colnames(PredictTable)[1]="Species"
    write.table(as.data.frame(PredictTable),paste("./bat-road-collision-risks/SIM_PREDICT/", Sp,"Acti_N_vehi_nuit_1200_predict_sim_table",".csv",sep=""),sep=";",row.names=F)
  ...
  if(grepl("H_veg", formula(BestModel)[3])){
    ...
    # Backtransformation of response
    pr3$predicted=exp(pr3$predictedRaw)
    ...
    colnames(PredictTable)[1]="Species"
    write.table(as.data.frame(PredictTable),paste("./bat-road-collision-risks/SIM_PREDICT/", Sp,"Acti_H_veg_predict_sim_table",".csv",sep=""),sep=";",row.names=F)
  ...
  if(grepl("Paysage", formula(BestModel)[3])){
    ...
    newdata = data.frame(JJulien_scaled=0,
                         ...
                         Site="bla",
                         SP="bli")
    newdata$Paysage=factor(newdata$Paysage, levels = c("NV","F", "FE", "DPT","SPT","PT"))
    ...
    # Backtransformation of response
    pr4$predicted=exp(pr4$predictedRaw)
    
    # Save table
    PredictTable=cbind(rep(Sp,nrow(pr4)),as.data.frame(pr4))
    colnames(PredictTable)[1]="Species"
    write.table(as.data.frame(PredictTable),paste("./bat-road-collision-risks/SIM_PREDICT/", Sp,"Acti_Paysage_predict_sim_table",".csv",sep=""),sep=";",row.names=F)
  ...
  if(grepl("JJulien", formula(BestModel)[3])){
    ...
    # Backtransformation of response
    pr5$predicted=exp(pr5$predictedRaw)
    ...
    colnames(PredictTable)[1]="Species"
    write.table(as.data.frame(PredictTable),paste("./bat-road-collision-risks/SIM_PREDICT/", Sp,"Acti_JJulien_predict_sim_table",".csv",sep=""),sep=";",row.names=F)
