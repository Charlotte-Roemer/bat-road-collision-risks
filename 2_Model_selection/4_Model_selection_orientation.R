...
ModSelection<- function (VarDispo,FormulaFix,FormulaRandom,familyMod) {
  ...
  AIC_prec=AICc(glmmTMB(Formula, data=MaDataOrientationNew, family=familyMod))
  ...
  if(length(VarSimple)>0){
    while(DeltaAIC>2)
    {
      ...
      for (h in 1:(length(VarDispo)))
      {
        ...
        ModTemp=glmmTMB(Formula, data=MaDataOrientationNew, family=familyMod)
        if (!is.nan(max(vif.mer(ModTemp)))){
#          if (max(vif.mer(ModTemp)[3:length(vif.mer(ModTemp))])<3){ # on enlève le JJulien et JJulien2 qui donnent de grands vif
